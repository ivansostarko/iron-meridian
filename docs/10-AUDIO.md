...
| Mine blast | **Synthesised** (`ProceduralAudio.Shell`) — or `Audio/effects/mine_blast.*` | `MineBlast` | No | 200 → 78 Hz over 0.20 s, 1.2 s clip, rumble almost switched off. A mine is *buried*: the earth takes the low end and returns a crack rather than a boom, and absorbs what a shell would have thrown into the air. Higher and shorter even than the UAV warhead. The point is that the player can tell "I have driven into something" from "I am being shelled" **without looking**, because the two call for opposite responses. (docs/31-OBSTACLES.md §6) |
| Shahed warhead | **Synthesised** (`ProceduralAudio.Shell`) — or `Audio/effects/shahed_warhead.*` | `ShahedWarheadBurst` | No | Between the 155 and the 203: 78 → 24 Hz over 1.1 s, 3.6 s clip. Fifty-odd kilograms is a shell's report, not a munition's crack. |
| Missile (light) | **Synthesised** (`ProceduralAudio.Shell`) — or `Audio/effects/missile_light.*` | `MissileLightBurst` | No | Above every tube in the game: 150 → 52 Hz over 0.5 s, open crack filter. |
| Missile (medium) | **Synthesised** (`ProceduralAudio.Shell`) — or `Audio/effects/missile_medium.*` | `MissileMediumBurst` | No | 74 → 22 Hz over 1.4 s, 4.4 s clip. The reference heavy warhead. |
| Missile (heavy) | **Synthesised** (`ProceduralAudio.Shell`) — or `Audio/effects/missile_heavy.*` | `MissileHeavyBurst` | No | 48 → 12 Hz over 2.4 s, 6.5 s clip — below the aerial bomb. The lowest and longest detonation in the game. |

Ids in code: `EffectSound.Fire` / `.Explosion` / `.Smoke` / `.Impact` / `.ArtilleryLight` / `.ArtilleryMortar` / `.ArtilleryMedium` / `.ArtilleryHeavy` / `.AerialBomb` / `.UavWarhead` / `.ShahedWarhead` / `.MissileLight` / `.MissileMedium` / `.MissileHeavy` / `.MineBlast` (and `.JetPass` / `.DroneBuzz` / `.ShahedEngine` / `.MissileMotor` / `.MissileIncoming`, which are not carried by effects — see §2.4).
...
|---|---|---|---|---|
| Jet pass | **Synthesised** (`ProceduralAudio.JetPass`) — or `Assets/Resources/Audio/effects/jet_pass.*` | `BomberRun.Launch` and `SupplyRun.Launch`, parented to the aircraft | No | Broadband roar swelling and fading over a turbine tone that slides 115 → 62 Hz. 6 s. |
| Drone buzz | **Synthesised** (`ProceduralAudio.DroneBuzz`) — or `Audio/effects/drone_buzz.*` | `DroneRun.Launch` and `ReconDroneRun.Launch`, parented to the drone | **Yes** | Five detuned blade-pass tones (118–237 Hz) over a thin hiss. The detuning is the trick: four propellers never turn at exactly the same rate, and the slow beating between them is what the ear reads as a multirotor rather than a wasp. Cut the instant the warhead goes off — or, on the reconnaissance drone, when it has flown home, so a departure sounds like a departure rather than a disappearance. |
| Shahed engine | **Synthesised** (`ProceduralAudio.ShahedEngine`) — or `Audio/effects/shahed_engine.*` | `DroneRun.Launch`, parented to the drone | **Yes** | A sawtooth at ~86 Hz with its first six harmonics and a firing rate that wavers a couple of per cent — a small two-stroke under load, which is the sound the class is named for. Deliberately nothing like `DroneBuzz`: the engine is the first cue telling you which drone is coming. |
| Missile motor | **Synthesised** (`ProceduralAudio.MissileMotor`) — or `Audio/effects/missile_motor.*` | `MissileRun.Launch`, parented to the missile | **Yes** | Broadband roar with a 41 Hz combustion core. No pitch slide, unlike the jet pass — the motor is heard *from* the missile, so the changing distance is the audio source's job rather than the clip's. |
| Missile incoming | **Synthesised** (`ProceduralAudio.MissileIncoming`) — or `Audio/effects/missile_incoming.*` | `MissileRun.Update`, at the **target**, 1.6 s before impact | No | A whistle rising 320 → 1350 Hz under a swelling roar; everything grows toward the impact and nothing decays. Played at the ground point rather than on the airframe — it is the sound of something arriving where you are looking, and riding it down would put it in the wrong place until the last instant. |

`EffectSound.JetPass` is **reused by the air supply transport** (docs/29-AIR-SUPPLY.md). A turboprop airlifter does not sound like a jet, and a dedicated synth would be more correct — but the cue's job is "an aircraft is passing low overhead", which this says, and a sound nobody asked for is not worth a new synth. It is a known gap rather than a decision anybody should be happy with.
