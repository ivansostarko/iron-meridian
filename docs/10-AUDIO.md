...
Ids in code: `EffectSound.Fire` / `.Explosion` / `.Smoke` / `.Impact` / `.ArtilleryLight` / `.ArtilleryMortar` / `.ArtilleryMedium` / `.ArtilleryHeavy` / `.AerialBomb` / `.UavWarhead` / `.ShahedWarhead` / `.MissileLight` / `.MissileMedium` / `.MissileHeavy` (and `.JetPass` / `.DroneBuzz` / `.ShahedEngine` / `.MissileMotor` / `.MissileIncoming`, which are not carried by effects — see §2.4).
