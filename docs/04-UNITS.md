...
The unit catalogue lives in `Assets/StreamingAssets/Data/units.json` (generated by `scripts/generate_units.py`); the formation names units are given on the map live beside it in `unit-names.json`, which is hand-edited — see **Formation names** below. The **same catalogue serves both teams** — team, affiliation and echelon are chosen at deployment. Every type has a custom APP-6-style icon for each team in `Assets/Resources/Icons/{Friendly,Enemy}/<id>.png` (blue rectangle = friendly frame, red diamond = hostile frame; generated by `scripts/generate_icons.py`).
...
Every unit on the map carries a name of its own — *3rd Lancers*, *12th Field
Battery*, *7th Sappers* — rather than the type name every unit of that type
shares. It is what the map label prints under the echelon indicator, and what
the info panel, the hover card, the DEPLOYED list and every order message call
the formation.

**The names are data.** They live in `Assets/StreamingAssets/Data/unit-names.json`,
beside `units.json`. Nothing generates that file — it is hand-edited, and editing
it is the whole of retuning this. There is no C# to change.

```jsonc
{
  "pattern": "{ordinal} {title}",
  "ordinals": ["1st", "2nd", "3rd", … "30th"],
  "groups": [
    { "key": "armour", "unitIds": [], "branches": ["Armour"],
      "titles": ["Lancers", "Hussars", "Dragoons", …] }
  ],
  "fallbackTitles": ["Detachment", …]
}
```

**A title is a name without an echelon.** The label already draws the echelon
indicator (`II`, `X`, …) on the line above, so `Lancers` is the right entry and
`3rd Armoured Battalion` is not.

**How a type finds its family**, in order:

1. the first group whose `unitIds` contains the type's `id` from `units.json`,
2. the first group whose `branches` contains its `branch`,
3. `fallbackTitles`.

So id groups are the specific case and branch groups are the net underneath
them — which is what lets the 33 types in the `Other` branch split into
engineers, signals, EW/cyber, intelligence, provost, CBRN and information-support
families instead of all drawing from one pool, and what keeps a medevac helicopter
out of the fixed-wing pool and a MANPADS team out of the radar one. Order inside each pass is file order, and the first claim
wins, so the specific families are written above the branch-wide ones. A group
may carry both lists. As shipped, all 117 types match a group and none reaches
the fallback.

**Names are unique across the map.** The generator walks `ordinal × title` from a
starting point hashed off the unit's instance id, and takes the first combination
no formation on the map is already using — so a group's pool is
`ordinals × titles` (30 × 8 = 240 for most of them). Exhaust a pool and it starts
suffixing `(2)`, which is ugly on purpose: it means that family needs more titles.

**A name is issued once and then kept.** It is written to `UnitState.customName`,
which is a saved field, so it survives a save/load round trip and the formation is
called the same thing tomorrow. That is also why naming only ever runs for a unit
whose `customName` is empty — a name from a save, or one the player typed, is not
something to overwrite. `UnitActor.Spawn` is the single choke point every route
onto the map goes through (palette drop, paste, reinforcement, save being
applied), so it is the only place this is called.

A **pasted** copy is the one case that deliberately drops the name it was cloned
from: two formations called the same thing is the one thing this exists to
prevent, so the paste clears `customName` and the copy is issued a fresh one,
exactly as it is issued a fresh instance id and dropped from its original group.

If the file is missing or unreadable the game says so in the console and every
unit keeps its type name — which is what it did before this existed.
